...
LOGICAL, SAVE :: skipbl=.false., clear=.true., echo=.false.,           &
    debug=.false., more
INTEGER, SAVE :: item=0, nitems=0, loc(0:1000)=0, end(1000)=0,               &
...
character(len=255), dimension(100), save :: blocks_read='', labels_read=''
...
PUBLIC :: item, nitems, read_line, stream, reada, readu, readl,        &
    ...
    parse, char, line, blocks_read, labels_read, rewind_file !, getargs
...
!  Logical line assembled. First look for input directives
  ...
  if (char(L:L) .eq. "#" .and. .false.) then !Liam added ".and..false" to disable input directives since I want # to start a comment
...
write(*,*)
write(*,'("*** ERROR reading input: ", A)') trim(adjustl(c))
if (present(reflect) .or. .true.) then
  if (reflect .or. .true.) then
    l=loc(item)
    i2=min(last,l+20)
    i1=max(1,i2-70)
    s1=" "
    ...
    if (level .gt. 0) then
      ...
      write(*,'(4X,"Input line ",I0,":")') line(level)
    endif
    !print "(a3,1x,a,1x,a3)", s1, char(i1:i2), s2
    write(*,'(4X,A)') trim(char) !trim(adjustl(char))

    print "(4x,80a1)", (" ", i=1,l-1), "^"
  end if
end if
...
DOUBLE PRECISION, INTENT(OUT) :: colour(3)
...
case("RGB")
  ...
  call readf(colour(3))
case("RGB255")
  call readf(colour(1),255.0d0)
  call readf(colour(2),255.0d0)
  call readf(colour(3),255.0d0)
case("RGBX")
  call readu(x)
  read (x(1:2),"(z2)") r
  colour(1)=r/255d0
